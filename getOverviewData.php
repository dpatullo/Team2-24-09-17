...
if($year == "3000" || !$year){
    $query = sprintf("SELECT total_transacted,start_date FROM ip17team2db.totals WHERE outlet_id = '".$outlet."'");
}else{
    $query = sprintf("SELECT total_transacted,start_date FROM ip17team2db.totals WHERE outlet_id = '".$outlet."' AND year(start_date) = '".$year."'");
}
